"""
Modal training script for NNUE chess engine - Simplified for direct function calls.

Usage:
  # Generate training data (detached)
  modal run --detach train_modal_final.py::generate_data --pgn-file data/lichess_elite_2021-01.pgn --num-positions 50000

  # Train model (detached)
  modal run --detach train_modal_final.py::train_model --data-file training_data.json --epochs 5
...
def generate_data(
    pgn_file: str = "data/lichess_elite_2021-01.pgn",
    ...
    # Find PGN file
    volume_pgn = Path(VOLUME_PATH) / pgn_file
    if volume_pgn.exists():
        pgn_path = volume_pgn
        print(f"Using PGN from volume: {pgn_path}")
    else:
        pgn_path = Path(pgn_file)
        if not pgn_path.exists():
            print(f"ERROR: PGN not found: {pgn_file}")
            print(f"Tried: {pgn_path} and {volume_pgn}")
            return 0
        print(f"Using local PGN: {pgn_path}")
